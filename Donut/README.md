...
This is a minimalist 3D ASCII donut animation that renders a rotating torus using character-based shading. The project includes two implementations: a compact C version that runs in the terminal and a Python version that uses Pygame to display the same effect in a colorful window. Both share the same core rendering math, just presented through very different mediums.
